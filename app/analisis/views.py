...
def index():
    if request.method == 'POST':
        ...
        session["nama_vektor"] = request.form['nama_vektor']
        session["linkage_method"] = request.form['linkage']
...
def secondcluster():
    ...
    nama = request.form['nama_model']
    jumlah = request.form['jumlah']
    linkage = request.form['linkage_sec']
    ...
    gambar(perta, request.form['nama_model'])


    pathfile = get_id('UPLOAD_BOKEH', request.form['nama_model'], '.pkl')

    with open(pathfile, 'wb') as f:
        pickle.dump(perta, f)

    return jsonify({ 'linkimg': request.form['nama_model'] })
...
@analisis.route('/cekvektor', methods=['GET', 'POST'])
@login_required
@permission_required(Permission.ANALISIS)
def cekvektor():
    nama_vektor = request.form['nama_vektor']

    adanama = Vectorspace.query.filter_by(nama=nama_vektor).first()
 
    if adanama is None:
        pesan = 'sukses'
    else:
        pesan = '   nama ' + adanama.nama + ' sudah terpakai'

    return jsonify({ 'pesan': pesan })


@analisis.route('/cekmodel', methods=['GET', 'POST'])
@login_required
@permission_required(Permission.ANALISIS)
def cekmodel():
    nama_model = request.form['nama_model']

    adanama = Klustermodel.query.filter_by(nama=nama_model).first()
...
def gambar(perta, name):
    ...
    p.circle("xdata",  "ydata", size=15, source=source,
